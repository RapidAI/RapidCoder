...
// Shell 相关处理
ipcMain.handle('start-shell', (event) => {
    if (shellProcess) {
        shellProcess.kill(); // 关闭已有的 Shell
    }
    // 启动一个新的 Shell
    shellProcess = spawn(process.platform === 'win32' ? 'cmd.exe' : 'bash');

    // 监听 Shell 输出
    shellProcess.stdout.on('data', (data) => {
        event.sender.send('shell-output', data.toString());
    });
    ...
    shellProcess.on('close', (code) => {
        event.sender.send('shell-output', `Shell 进程退出，代码: ${code}`);
        shellProcess = null;
    });

    return { success: true, message: 'Shell 已启动' };
});

ipcMain.handle('send-shell-command', (event, command) => {
    if (shellProcess) {
        shellProcess.stdin.write(command + '\n');
    } else {
        event.sender.send('shell-output', 'Shell 未启动，请先启动 Shell');
    }
});

ipcMain.handle('stop-shell', () => {
    if (shellProcess) {
        shellProcess.kill();
        shellProcess = null;
    }
