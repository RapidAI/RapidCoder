...
const initMainWindow = () => {
    ...
    // 创建右侧视图
    const rightView = new BrowserWindow({
        parent: win,
        width: 800,
        height: 800,
        show: false, // 作为子窗口不显示
    ...
    rightView.loadURL('https://www.baidu.com');

    // 显示右侧页面在左侧窗口中
    win.webContents.on('did-finish-load', () => {
        win.addTabbedWindow(rightView); // 通过tab显示右侧内容
    });
    ...
    // 绑定快捷键或触发事件来刷新视图
    ipcMain.handle('refresh-views', refreshViews);
};
